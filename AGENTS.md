...
- **Project documentation is linked, not embedded.** The PDF sits in `public/docs/<project>/` and gets an `AppLink` in the `tryOutLink` style (`DOKU ANSEHEN` / `VIEW DOCUMENTATION`). AppLink opens `.pdf` hrefs in a new tab, so the browser's own PDF viewer shows it. An in-page reader was built and dropped on purpose (it needed hundreds of rendered page images) — don't reintroduce one.
...
1. `public/robots.txt` — a request. Search engines get `Allow: /`; a second group lists the AI crawler tokens and disallows `/de/project/`, `/en/project/`, `/img/`, `/docs/kai/` and `/docs/beacon/`, with `Allow: /img/cv/` so the CV imagery stays indexable.
