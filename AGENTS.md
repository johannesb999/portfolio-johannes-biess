...
- **`DocumentReader.vue`** — reads a document rendered to single images (`public/docs/<project>/pages/<locale>/NNN.webp`); a visitor loads only what they look at (50–70 KB per page) instead of the whole file. The frame has a fixed height (`72vh`, `64vh` stacked) and scrolls **inside itself** — the project page must never grow to the length of the document. Two modes, switched by text links, no button chrome:
  - **The page images are not in the repo.** `scripts/render-docs.mjs` renders them from the PDFs next to them, and runs by itself before `npm run dev`, `build` and `generate`. `public/docs/*/pages/` is gitignored — never commit it. A new document goes into the `DOCUMENTS` list of that script; page count and `ratio` go on its `<DocumentReader>`. Both mupdf and the WebP encoder are WebAssembly, so the Alpine Docker build needs no native toolchain. Unchanged PDFs are skipped (checksums in `node_modules/.cache/render-docs`); `npm run docs -- --force` re-renders everything.
  ...
  - `.project-doc` breaks out of the content column to `$media-width` (85vw, shared with `.project-film`) so the pages are actually readable, and needs `grid-template-columns: minmax(0, 1fr)` — otherwise the grid item's automatic minimum size pushes the reader past the viewport.
